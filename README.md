...
```json
{
    ...
    "data": {
        ...
        "gitut $a": "version=$(v=`git describe --abbrev=0 --tags 2>/dev/null`;n=(${v//./ });n1=${n[0]};n2=${n[1]};n3=${n[2]};if [ -z \"$n1\" ] && [ -z \"$n2\" ] && [ -z \"$n3\" ]; then n1=1; n2=0; n3=0;else n3=$((n3+1)); fi;if [ \"$n3\" == \"10\" ]; then n3=0; n2=$((n2+1)); fi;if [ \"$n2\" == \"10\" ]; then n2=0; n1=$((n1+1)); fi;echo \"$n1.$n2.$n3\") && if [ -f package.json ]; then npm --no-git-tag-version version \"$version\"; fi && git add -A . && git commit -m \"$a\" && git push origin HEAD && git tag -a \"$version\" -m \"$a\" && git push --tags\r",
        "gitrm $a": "git rm -rf --cached \"$a\"",
        "gitu": "git add -A . && git commit -m \".\" && git push origin HEAD\r",
        "gitut": "version=$(v=`git describe --abbrev=0 --tags 2>/dev/null`;n=(${v//./ });n1=${n[0]};n2=${n[1]};n3=${n[2]};if [ -z \"$n1\" ] && [ -z \"$n2\" ] && [ -z \"$n3\" ]; then n1=1; n2=0; n3=0;else n3=$((n3+1)); fi;if [ \"$n3\" == \"10\" ]; then n3=0; n2=$((n2+1)); fi;if [ \"$n2\" == \"10\" ]; then n2=0; n1=$((n1+1)); fi;echo \"$n1.$n2.$n3\") && if [ -f package.json ]; then npm --no-git-tag-version version \"$version\"; fi && git add -A . && git commit -m \".\" && git push origin HEAD && git tag -a \"$version\" -m \".\" && git push --tags\r",
