...
```json
{
    ...
    "data": {
        ...
        "audit": "clear && printf \"\\n\\n\"; max=0; for d in ./*/; do n=$(basename \"$d\"); [ \"$n\" = \"_archive\" ] && continue; [ ! -d \"$d/.git\" ] && continue; [ ${#n} -gt $max ] && max=${#n}; done; for d in ./*/; do n=$(basename \"$d\"); [ \"$n\" = \"_archive\" ] && continue; [ ! -d \"$d/.git\" ] && continue; npm_ok=0; comp_ok=0; phprc_ok=0; nvmrc_ok=0; mod_ok=0; sync_ok=0; if [ -f \"$d/package.json\" ]; then nout=$(cd \"$d\" && npm outdated --json 2>/dev/null); [ -n \"$nout\" ] && echo \"$nout\" | jq -e '[to_entries[] | select(.value.wanted != .value.latest)] | length > 0' >/dev/null 2>&1 && npm_ok=1; elif [ -f \"$d/wp-content/themes/$n/package.json\" ]; then nout=$(cd \"$d/wp-content/themes/$n\" && npm outdated --json 2>/dev/null); [ -n \"$nout\" ] && echo \"$nout\" | jq -e '[to_entries[] | select(.value.wanted != .value.latest)] | length > 0' >/dev/null 2>&1 && npm_ok=1; fi; if [ -f \"$d/composer.json\" ]; then (cd \"$d\" && php8.5 $(which composer) outdated -D --strict --major-only >/dev/null 2>&1) || comp_ok=1; elif [ -f \"$d/wp-content/themes/$n/composer.json\" ]; then (cd \"$d/wp-content/themes/$n\" && php8.5 $(which composer) outdated -D --strict --major-only >/dev/null 2>&1) || comp_ok=1; fi; if [ -f \"$d/.phprc\" ]; then [ \"$(cat \"$d/.phprc\")\" = \"8.5\" ] || phprc_ok=1; elif [ -f \"$d/wp-content/themes/$n/.phprc\" ]; then [ \"$(cat \"$d/wp-content/themes/$n/.phprc\")\" = \"8.5\" ] || phprc_ok=1; fi; if [ -f \"$d/.nvmrc\" ]; then [ \"$(cat \"$d/.nvmrc\")\" = \"lts/*\" ] || nvmrc_ok=1; elif [ -f \"$d/wp-content/themes/$n/.nvmrc\" ]; then [ \"$(cat \"$d/wp-content/themes/$n/.nvmrc\")\" = \"lts/*\" ] || nvmrc_ok=1; fi; s=$(git -C \"$d\" status --porcelain); [ -n \"$s\" ] && mod_ok=1; a=0; b=0; if git -C \"$d\" rev-parse --abbrev-ref --symbolic-full-name '@{u}' >/dev/null 2>&1; then timeout 10s git -C \"$d\" fetch --all --prune >/dev/null 2>&1 || true; a=$(git -C \"$d\" rev-list --count --left-only HEAD...@{u}); b=$(git -C \"$d\" rev-list --count --right-only HEAD...@{u}); fi; if [ \"$a\" -gt 0 ] || [ \"$b\" -gt 0 ]; then sync_ok=1; fi; if [ \"$npm_ok\" -eq 0 ] && [ \"$comp_ok\" -eq 0 ] && [ \"$phprc_ok\" -eq 0 ] && [ \"$nvmrc_ok\" -eq 0 ] && [ \"$mod_ok\" -eq 0 ] && [ \"$sync_ok\" -eq 0 ]; then printf \"✅ %-${max}s\\n\" \"$n\"; else m=\"\"; [ $npm_ok -eq 1 ] && m=\"${m}npm, \"; [ $comp_ok -eq 1 ] && m=\"${m}composer, \"; [ $phprc_ok -eq 1 ] && m=\"${m}php, \"; [ $nvmrc_ok -eq 1 ] && m=\"${m}nvm, \"; [ $mod_ok -eq 1 ] && m=\"${m}modified, \"; [ $sync_ok -eq 1 ] && m=\"${m}behind/ahead, \"; printf \"⛔ %-${max}s  [%s]\\n\" \"$n\" \"${m%, }\"; fi; done\r",
        "cfix": "clear:both;\ndisplay:table;\ncontent:\"\"",
        ...
        "opencode": "OPENCODE_DISABLE_CLAUDE_CODE=true OPENCODE_DISABLE_EXTERNAL_SKILLS=true IS_SANDBOX=1 opencode $([ -n \"$(OPENCODE_DISABLE_CLAUDE_CODE=true OPENCODE_DISABLE_EXTERNAL_SKILLS=true opencode session list --format json --max-count 1 2>/dev/null)\" ] && echo --continue) --auto --model opencode-go/glm-5.2\r",
