...
```json
{
    ...
    "data": {
        ...
        "gitut": "git add -A . && git commit -m \".\" && git push origin HEAD && git tag -a $(v=`git describe --abbrev=0 --tags 2>/dev/null`;n=(${v//./ });n1=${n[0]};n2=${n[1]};n3=${n[2]};if [ -z \"$n1\" ] && [ -z \"$n2\" ] && [ -z \"$n3\" ]; then n1=1; n2=0; n3=0;else n3=$((n3+1)); fi;if [ \"$n3\" == \"10\" ]; then n3=0; n2=$((n2+1)); fi;if [ \"$n2\" == \"10\" ]; then n2=0; n1=$((n1+1)); fi;echo \"$n1.$n2.$n3\") -m \"$a\" && git push --tags\r",
        ...
        "aioff": "Schreibe ein Handoff-Dokument nach /tmp/HANDOFF-<projekt>.md, wobei <projekt> der Verzeichnisname des Git-Roots (bzw. des aktuellen Arbeitsverzeichnisses) ist – ermittle ihn selbst und überschreibe eine ggf. vorhandene Datei. Beginne das Dokument mit einem Metadaten-Block: absoluter Projektpfad, Branch, Datum/Uhrzeit, kurzes Thema in einem Satz. Dann: 1) Aufgabe & Ziel, 2) Aktueller Stand (was ist fertig, was verifiziert, wie getestet), 3) Getroffene Entscheidungen mit Begründung, 4) Verworfene Ansätze und warum, 5) Geänderte Dateien (Pfade, relevante Commits/Branch – Diffs nur referenzieren, nicht einfügen), 6) Offene Punkte & bekannte Risiken, 7) Konkrete nächste Schritte. Falls die Datei bereits existiert und von einem anderen Agenten stammt: Gehe in einem Abschnitt 'Antwort auf vorheriges Handoff' explizit auf dessen Kritikpunkte ein – was du übernommen hast, was du begründet ablehnst. Sei ehrlich statt beschönigend: ungelöste Probleme und Unsicherheiten explizit nennen. Keine Secrets, Tokens oder Zugangsdaten ins Dokument schreiben. Gib am Ende den vollständigen Dateipfad aus.\r",
        "aion": "Ermittle den Verzeichnisnamen des Git-Roots (bzw. des aktuellen Arbeitsverzeichnisses) und lies /tmp/HANDOFF-<projekt>.md. Validiere zuerst den Metadaten-Block: Stimmt der Projektpfad mit dem aktuellen Verzeichnis überein und passt das Thema zum aktuellen Kontext? Falls die Datei fehlt oder zu einem anderen Projekt gehört: Brich ab und melde das – arbeite dann NICHT mit dem Inhalt. Falls der Zeitstempel älter als ein Tag ist: Weise explizit darauf hin und frag nach, bevor du weiterarbeitest. Andernfalls: Das Handoff stammt von einem anderen KI-Agenten und kann Fehler, Lücken oder zu optimistische Einschätzungen enthalten. Verifiziere die zentralen Behauptungen selbst gegen den tatsächlichen Code (Dateien lesen, Diff/Commits prüfen), übernimm nichts ungeprüft. Bewerte dann kritisch: Sind die getroffenen Entscheidungen sinnvoll? Gibt es Fehler, Inkonsistenzen oder übersehene Probleme? Sind die offenen Punkte vollständig? Liefere als Ergebnis: a) Abweichungen zwischen Handoff und realem Code, b) gefundene Probleme mit Datei/Zeile und Schweregrad, c) deine Empfehlung für das weitere Vorgehen.\r",        "lorem": "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu, pretium quis, sem. Nulla consequat massa quis enim. Donec pede justo, fringilla vel, aliquet nec, vulputate eget, arcu. In enim justo, rhoncus ut, imperdiet a, venenatis vitae, justo. Nullam dictum felis eu pede mollis pretium. Integer tincidunt. Cras dapibus. Vivamus elementum semper nisi. Aenean vulputate eleifend tellus.",
