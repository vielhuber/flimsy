...
```json
{
    ...
    "data": {
        ...
        "audit": "for d in ./*/; do n=$(basename \"$d\"); [ \"$n\" = \"_archive\" ] && continue; npm_ok=0; comp_ok=0; if [ -f \"$d/package.json\" ]; then (cd \"$d\" && npm outdated >/dev/null 2>&1) || npm_ok=1; elif [ -f \"$d/wp-content/themes/$n/package.json\" ]; then (cd \"$d/wp-content/themes/$n\" && npm outdated >/dev/null 2>&1) || npm_ok=1; fi; if [ -f \"$d/composer.json\" ]; then (cd \"$d\" && composer outdated -D --strict >/dev/null 2>&1) || comp_ok=1; elif [ -f \"$d/wp-content/themes/$n/composer.json\" ]; then (cd \"$d/wp-content/themes/$n\" && composer outdated -D --strict >/dev/null 2>&1) || comp_ok=1; fi; if [ \"$npm_ok\" -eq 0 ] && [ \"$comp_ok\" -eq 0 ]; then echo \"✅ $n\"; else echo \"⛔ $n\"; fi; done\r",
        "cfix": "clear:both;\ndisplay:table;\ncontent:\"\"",
